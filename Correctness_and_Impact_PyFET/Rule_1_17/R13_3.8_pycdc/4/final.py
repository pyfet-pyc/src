...
# Compiled at: 2022-08-13 08:31:01
...
class AdobePassIE(InfoExtractor):
    ...
    def _extract_mvpd_auth(self, url, video_id, requestor_id, resource):
        ...
        def post_form(form_page_res, note, data={}):
            ...
            return self._download_webpage_handle(post_url,
              video_id, note, data=(urlencode_postdata(form_data)), headers={'Content-Type': 'application/x-www-form-urlencoded'})

        def raise_mvpd_required():
            raise ExtractorError('This video is only available for users of participating TV providers. Use --ap-mso to specify Adobe Pass Multiple-system operator Identifier and --ap-username and --ap-password or --netrc to provide account credentials.',
              expected=True)

        def extract_redirect_url(html, url=None, fatal=False):
            REDIRECT_REGEX = '[0-9]{,2};\\s*(?:URL|url)=\\\'?([^\\\'"]+)'
            redirect_url = self._search_regex(('(?i)<meta\\s+(?=(?:[a-z-]+="[^"]+"\\s+)*http-equiv="refresh")(?:[a-z-]+="[^"]+"\\s+)*?content="%s' % REDIRECT_REGEX),
              html,
              'meta refresh redirect', default=(NO_DEFAULT if fatal else None),
              fatal=fatal)
            if not redirect_url:
                return
            if url:
        ...
        mvpd_headers = FET_set('anonymous', 'Linux i686', self._USER_AGENT, self._USER_AGENT)
