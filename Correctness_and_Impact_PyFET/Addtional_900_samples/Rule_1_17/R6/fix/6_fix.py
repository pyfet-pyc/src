...
def foo():
  return {name: (fun, fun.__np_wrapped__) for name, fun in jnp_funcs.items()
                  if getattr(fun, '__np_wrapped__', None) is not None}

def foo_1():
  return {a: p for a, p in jnp_params.items() if a not in extra}

def foo_2():
  return {a: p for a, p in np_params.items() if a not in unsupported}