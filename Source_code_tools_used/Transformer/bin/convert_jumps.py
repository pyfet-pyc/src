...
from manipulate_pyc import get_co_fn, add_dummy_at_offset, break_boolean_chains
...
def instrument_code_at_offset():
    ...
    lst = break_boolean_chains(co, fn_name)
    print(lst)
    for i, x in enumerate(lst):
        if x is None: continue
        write_file(input_f, "{}/{}_{}jump.pyc".format(output_f,os.path.splitext(os.path.basename(input_f))[0],i), x)
