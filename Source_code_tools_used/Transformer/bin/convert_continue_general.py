...
from manipulate_pyc import get_co_fn, add_dummy_at_offset, convert_continue_general
...
def instrument_code_at_offset():
    ...
    lst = convert_continue_general(co, fn_name)
    print(lst)
    while 1:
        new_list = []
        for co_curr_2 in lst:
            new_list = convert_continue_general(co_curr_2, fn_name)
        if len(new_list) == 0: 
            break
        lst = new_list
    # APPLY TO ALL END
