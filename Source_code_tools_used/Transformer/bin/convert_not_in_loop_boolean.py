...
from manipulate_pyc import get_co_fn, add_dummy_at_offset, convert_not_in_loop_boolean
...
def instrument_code_at_offset():
    ...
    lst = convert_not_in_loop_boolean(co, fn_name)
    print(lst)
    for i, x in enumerate(lst):
        if x is None: continue
        write_file(input_f, "{}/{}_{}not_loop.pyc".format(output_f,os.path.splitext(os.path.basename(input_f))[0],i), x)
