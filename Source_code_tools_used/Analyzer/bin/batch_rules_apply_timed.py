...
def main():
    ...
    for f_id in dict_file_and_functions:
        ...
        lst = [co]
        logging.info("===========")
        logging.info("%s:%d bytes"%(f_id, os.path.getsize(f_input)))
        disable_print()
        for idx, fn_name in enumerate(dict_file_and_functions[f_id][0]):
            tmp_list = []
            tmp = convert_return_to_store_try_except(co, fn_name)
            lst_func = [break_boolean_chains, remove_dead_code, convert_loopBoolean, instrument_after_break, instrument_after_continue, instrument_after_poptop_at_the_end_of_loop,convert_return_to_store_try_except, convert_return_to_store_try_except_second, convert_continue_in_except, move_return_with_block_outside,convert_return_with_block, convert_breaks_with_variable_in_except_block, convert_break_if_last_instruction_in_loop, convert_continue_with_try_block, convert_continue_general]
            # disable_print()
            # try:
            #     start_time = time.time()
            #     tmp = uncompyle6.code_deparse(co,out=os.devnull)
            # except:
            #     pass
            # # print(tmp)
            # # logging.info(tmp)
            # enable_print()
            # logging.info("Decompile:%s" % (time.time() - start_time))
            disable_print()
            for fn in lst_func:
                co_recv = []
                try:
                    start_time = time.time()
                    co_recv = fn(co, fn_name)
                    enable_print()
                    if len(co_recv)>0: logging.info("%s %d" % (time.time() - start_time, len(co_recv)))
                except:
                    pass    
                disable_print()
