...
'BINARY_MATRIX_MULTIPLY','UNARY_INVERT', 'INPLACE_MATRIX_MULTIPLY', 'BINARY_POWER', 'INPLACE_FLOOR_DIVIDE', 'BINARY_TRUE_DIVIDE',
# functions
'MAKE_FUNCTION', 'CALL_FUNCTION', 'CALL_FUNCTION_KW', 'CALL_FUNCTION_EX', 'RETURN_VALUE', 'EXTENDED_ARG', 'LOAD_METHOD', 'CALL_METHOD', 'MAKE_FUNCTION',
# class
'LOAD_BUILD_CLASS',
# stack
...
def traverse(graph, nodes, index):
  visited = set()
  index_mark = []
  count = 0
  nodes = [(0,0)]
#   print('traversing now', nodes)
  while nodes:
    node, count = nodes.pop(index)
    if node not in visited:
      visited.add(node)
      index_mark += [(node, count)]
    #   callback(node)
      n = graph.get_node(node)
    #   n.attr['label'] = '%d\n' %(count) + n.attr['label']
      lst = graph.successors(node)
      lst = [(i, count+1) for i in lst]
      nodes.extend(lst)
  max= -1
  for node, count in index_mark:
    n = graph.get_node(node)
    n.attr['label'] = '%d\n' %(count) + n.attr['label']
    # print(n.attr)
    if count>max: max = count
  print(max, "COUNT!!!")
  print ([i for i,count in index_mark if count == max ])
  print ('traversed:',len([i for i,count in index_mark if count <= 35 ]))
  
  return n


def bfs(graph, nodes, callback=NOOP):
  return traverse(graph, nodes, INDEX_BFS)
